---
description: Interview stakeholders and create a business-ready task definition for technical research
agent: build
---

# Task Define: $ARGUMENTS

## Purpose

Produce a concise, implementation-neutral business definition that a human can review and `/task-research` can later investigate. Diagnose the problem and clarify the desired outcome; do not recommend architecture, code, libraries, APIs, data models, or implementation steps.
...
Set status to `READY_FOR_RESEARCH` only when the problem, desired outcome, acceptance criteria, in-scope items, out-of-scope items, and material limitations are clear. Otherwise set it to `DRAFT` and preserve the blocking open questions.

```markdown
# [TASK_ID]: [Task Title]

## Status
- Status: DRAFT / READY_FOR_RESEARCH
...
- Research must preserve: [scope, criteria, constraints, and decisions]
```
...
4. Open questions that prevent research, if any.
5. If ready: `Run /task-research $ARGUMENTS to investigate the current technical situation.`
